...
void GazeboRosOpenniKinect::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("encoding")) {
    this->encoding_type_ = "16UC1";
  } else {
    this->encoding_type_ = _sdf->GetElement("encoding")->Get<std::string>();
  }
    ...
    uint32_t step_arg, void* data_arg)
{
  ...
  float* toCopyFrom = (float*)data_arg;
  int index = 0;
  if (this->encoding_type_ == "16UC1") {
    image_msg.encoding = sensor_msgs::image_encodings::TYPE_16UC1;
    image_msg.step = sizeof(unsigned short) * cols_arg;
    image_msg.data.resize(rows_arg * cols_arg * sizeof(unsigned short));
    const unsigned short bad_point = std::numeric_limits<unsigned short>::quiet_NaN();
    unsigned short* dest = (unsigned short*)(&(image_msg.data[0]));
    // convert depth to point cloud
    for (uint32_t j = 0; j < rows_arg; j++) {
      for (uint32_t i = 0; i < cols_arg; i++) {
        ...
        if (depth > this->point_cloud_cutoff_ &&
          ...
          // convert from [m] to [mm]
            dest[i + j * cols_arg] = (unsigned short)((depth * 1000.0) + 0.5);
        } else {
  ...
  } else if (this->encoding_type_ == "32FC1") {
